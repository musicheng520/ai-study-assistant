...
| --- | --- |
| `400` | `INVALID_REQUEST`, `VALIDATION_FAILED`, `AI_OUTPUT_INVALID`, `INVALID_DRAFT_KEY`, `INVALID_DRAFT_SCOPE`, `INVALID_DRAFT_TYPE`, `INVALID_DRAFT_VALUE`, `DOCUMENT_NOT_READY`, `NO_READY_DOCUMENTS` |
| `401` | `UNAUTHORIZED` |
| `403` | `FORBIDDEN`, `COURSE_ACCESS_DENIED`, `DOCUMENT_ACCESS_DENIED`, `FORBIDDEN_DRAFT` |
| `404` | `COURSE_NOT_FOUND`, `DOCUMENT_NOT_FOUND`, `SUMMARY_NOT_FOUND`, `QUIZ_NOT_FOUND`, `FLASHCARD_NOT_FOUND`, `DRAFT_NOT_FOUND` |
...
| Workflow type | `RAG_QA`, `SUMMARY`, `QUIZ`, `FLASHCARD`, `ASSIGNMENT_ANALYSIS`, `RUBRIC_ANALYSIS`, `REVISION_PLAN`, `CHECKLIST` |
| Feedback target type | `SUMMARY`, `QUIZ`, `FLASHCARD`, `ASSIGNMENT_ANALYSIS`, `RUBRIC_ANALYSIS`, `REVISION_PACK` |
...
Generation endpoints return a `draftKey`. Save endpoints persist generated content by sending that `draftKey`.

Common save request:

```json
{
  "draftKey": "summary:1:1:course:abc123"
}
```
...
3. For generated content, call `generate`, keep the returned `draftKey`, then call the matching `save` endpoint when the user accepts it.
